...
2. **Add an LLM provider**: Settings, then Providers, pick one, paste your API key. **OpenRouter** is first-class (auto-detected model capabilities and pricing, provider routing controls). You can also skip API keys and sign in with a subscription instead: **Codex** with your ChatGPT account (your OpenAI plan over OAuth), or **Grok** with your xAI account (SuperGrok or X Premium+ over OAuth, with weekly-usage display and, on the image screen, Grok Imagine image generation). For fully on-device inference with no key and no network, open the **Local · LiteRT** provider and download a local model (Gemma, Qwen) — it runs on any device and uses the GPU automatically where supported. Multimodal Gemma builds can also read images you attach, decoded on-device where the device's vision encoder is supported (otherwise the model replies from text only). Pixel 8/9/10 users can also flip on the built-in **AICore** card for Gemini Nano.
...
| Storage      | ~80 MB app                                                   |
| LLM provider | OpenAI, Google, Anthropic, OpenRouter, Codex (ChatGPT OAuth), Grok (xAI OAuth), Ollama, or any OpenAI-compatible endpoint. OR Gemini Nano via AICore on Pixel 8/9/10+ |
