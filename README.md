...
**🤖 设备端 Agent 工具** —— 140+ 设备工具按需启用，一句话完成手机自动化。
...
> <span style="color:red">**❗️❗️❗️ 注：包含 140+ 工具，请按需启用 —— 常驻过多会持续占用上下文与额度！**</span>
...
- **工具按需注入**：工具面按名称稳定排序后注入请求前缀，并支持「冷档」—— 未解锁的工具只发空 schema，避免 140+ 工具一次性吃满上下文。
...
一个把原生 Android LLM 聊天客户端变成设备端 Agent 的项目：**140+ 设备工具**、AI 驱动的工作流、定时任务、内置浏览器（AI 操控）、SSH、屏幕自动化、文件管理、音乐播放、语音转文字、可下载的本地 LLM，以及远程 Telegram Bot。**所有功能默认关闭，按需开启。**
...
### 设备控制
点击、滑动、滚动、打字、截图、打开应用、调节亮度/音量、发送通知、检查电池/WiFi/信号/位置/传感器、读取联系人 & 短信、发送短信、设置壁纸、读写 NFC、管理 ZIP 压缩包。**140+ 工具**，全部默认关闭。
