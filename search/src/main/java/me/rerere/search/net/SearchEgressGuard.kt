...
// This intentionally parallels me.rerere.rikkahub.data.ai.net.GuardedDns in the app module:
// :search cannot depend on :app, so the scrape() egress guard for SearXNG/Bing is duplicated
 ...
 * literals are inspected, so no DNS query happens here; a hostname returns false. A null or
 * blank host also returns false: it is not a literal, and such a host only arises from an
 * unparseable URL that never dials. A malformed literal is refused (fail closed) rather
 * than throwing.
 */
