...
class CronBootReceiver :
        ...
        scope.launch {
            try {
                ...
                if (cfg != null && cfg.isUsable) {
                    me.rerere.rikkahub.service.TelegramBotService
                        .start(context)
                    me.rerere.rikkahub.service.TelegramBotHealthWorker
                        .schedule(context)
                }
                ...
                runCatching {
                    me.rerere.rikkahub.workflow.trigger.WorkflowBootDispatcher
                        .onBoot()
                }
            } finally {
                pending.finish()
    ...
    private suspend fun sweepStrandedRunRows(context: Context) {
        ...
        val title =
            if (stranded.size == 1) {
                "Scheduled job interrupted"
            } else {
                "${stranded.size} scheduled jobs interrupted"
            }
        val text =
            if (jobNames.size <= 3) {
                jobNames.joinToString(", ")
            } else {
                jobNames.take(3).joinToString(", ") + ", and ${jobNames.size - 3} others"
            }
    ...
    private fun postFailureNotification(
        ...
        val builder =
            androidx.core.app.NotificationCompat
                .Builder(ctx, CronJobWorker.CHANNEL_ID)
        ...
        try {
            ...
            androidx.core.app.NotificationManagerCompat
                .from(ctx)
                .notify(aggregateNotifId, builder.build())
