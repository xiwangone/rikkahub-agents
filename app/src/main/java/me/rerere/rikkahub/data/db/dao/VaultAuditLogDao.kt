...
interface VaultAuditLogDao {
    @Query("SELECT * FROM vault_audit_log ORDER BY ts_ms DESC LIMIT :limit")
    ...
    @Query("DELETE FROM vault_audit_log WHERE ts_ms < :cutoffMs")
    suspend fun deleteOlderThan(cutoffMs: Long)

    /** 只保留最新 keep 条，删除其余更旧的。 */
    @Query("DELETE FROM vault_audit_log WHERE id NOT IN (SELECT id FROM vault_audit_log ORDER BY ts_ms DESC LIMIT :keep)")
