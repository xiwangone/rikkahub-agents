...
internal class GeofenceTriggerFamily(
    ...
    override suspend fun sync(
        ...
        if (!hasGeofencePermissions()) {
            AppLog.w(TAG, "geofence: missing FINE_LOCATION/BACKGROUND_LOCATION; skipping registration")
        ...
        if (client == null) {
            AppLog.w(TAG, "geofence: Play Services unavailable; skipping registration")
            return
        }
        ...
        if (toRemove.isNotEmpty()) {
            withContext(Dispatchers.IO) {
                runCatching { Tasks.await(client.removeGeofences(toRemove.toList())) }
                    .onFailure { AppLog.w(TAG, "geofence: removeGeofences failed", it) }
            }
        }
            ...
            if (list.isNotEmpty()) {
                ...
                withContext(Dispatchers.IO) {
                    runCatching { Tasks.await(client.addGeofences(req, geofencingPendingIntent())) }
                        .onFailure { AppLog.w(TAG, "geofence: addGeofences failed", it) }
                }
            }
        }
        registered = targetMap
        AppLog.d(TAG, "geofence: synced (${targetMap.size} active, +${toAdd.size}, -${toRemove.size})")
    }
    ...
    private fun removeAll() {
        ...
        runCatching { Tasks.await(client.removeGeofences(registered.keys.toList())) }
            .onFailure { AppLog.w(TAG, "geofence: removeAll failed", it) }
        registered = emptyMap()
    }
            ...
            for (id in workflowIds) {
                ...
                if (matches) {
                    runCatching { cb.onFire(id, fireSpec) }.onFailure {
                        AppLog.w(TAG, "geofence: fire callback failed for wf=$id", it)
                    }
                }
            }
        }
...
object GeofenceTriggerDispatcher {
    ...
    fun onEvent(
        ...
        if (scope == null) {
            AppLog.w(TAG, "geofence: cold-start dispatch but AppScope unavailable; dropping ${workflowIds.size} event(s)")
            pendingResult?.finish()
            return
        }
        scope.launch(Dispatchers.IO) {
                ...
                workflowIds.map { id ->
                    launch {
                        runCatching { GeofenceTriggerHelper.lookupAndFire(id, transition) }
                            .onFailure { AppLog.w(TAG, "geofence: cold-start dispatch failed for wf=$id", it) }
                    }
                }
            try {
