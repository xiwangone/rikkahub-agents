...
object SolarTimes {
    ...
    fun sunriseAt(
        ...
        lng: Double,
        zone: ZoneId,
    ): LocalTime? = compute(date, lat, lng, zone, sunrise = true)
    ...
    fun sunsetAt(
        ...
        lng: Double,
        zone: ZoneId,
    ): LocalTime? = compute(date, lat, lng, zone, sunrise = false)
    ...
    private fun compute(
        ...
        // 6. Local hour angle.
        val cosH =
            (cos(ZENITH_DEG.deg()) - (sinDec * sin(lat.deg()))) /
                (cosDec * cos(lat.deg()))
        ...
        val utc =
            date
                .atTime(utHoursInt.coerceIn(0, 23), utMinutesInt.coerceIn(0, 59))
