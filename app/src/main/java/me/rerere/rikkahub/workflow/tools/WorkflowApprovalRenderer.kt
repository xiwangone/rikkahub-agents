...
object WorkflowApprovalRenderer {
        ...
        sb.appendLine(
            "Only if: ${
                if (def.conditions.isEmpty()) {
                    ...
                    def.conditions.joinToString(", ") { conditionSummary(it) }
                }
            }",
        )
        sb.appendLine("Do:")
    ...
    fun renderTelegramHtml(
        ...
        sb.appendLine(
            "<b>Only if:</b> ${
                if (def.conditions.isEmpty()) {
                    ...
                    def.conditions.joinToString(", ") { escapeHtml(conditionSummary(it)) }
                }
            }",
        )
        ...
        for ((idx, action) in def.actions.withIndex()) {
            sb.appendLine(
                "  ${idx + 1}. <code>${escapeHtml(action.tool)}</code>(${escapeHtml(actionArgsHint(action))})",
            )
        }
        sb.appendLine()
        sb.appendLine(
    ...
    private fun triggerSummary(t: TriggerSpec): String =
        when (t) {
            is TriggerSpec.TimeCron -> {
                when {
                    !t.timeOfDay.isNullOrBlank() -> {
                        val days =
                            if (t.daysOfWeek.isEmpty()) {
                                "every day"
                            } else {
                                "on day(s) ${t.daysOfWeek.joinToString(
                                    ",",
                                )}"
                            }
            ...
            is TriggerSpec.BluetoothDeviceConnected -> {
                "Bluetooth device connects" +
                    (t.deviceAddress?.let { " ($it)" }.orEmpty())
            }
            ...
            is TriggerSpec.BatteryBelow -> {
                "battery drops below ${t.thresholdPercent}%"
            }

            is TriggerSpec.BatteryAbove -> {
                "battery rises above ${t.thresholdPercent}%"
            }

            is TriggerSpec.GeofenceEnter -> {
                "you enter ${t.label ?: "a place (${t.lat},${t.lng}, ${t.radiusM}m)"}"
            }

            is TriggerSpec.GeofenceExit -> {
                "you leave ${t.label ?: "a place (${t.lat},${t.lng}, ${t.radiusM}m)"}"
            }

            is TriggerSpec.AppLaunched -> {
                "${t.packageName} is launched"
            }

            is TriggerSpec.AppClosed -> {
                "${t.packageName} is closed"
            }
        ...
        val base =
            when (c) {
                ...
                is ConditionSpec.ForegroundAppIs -> {
                    "${c.packageName} is foreground"
                }

                is ConditionSpec.ForegroundAppIn -> {
                    "${c.packageNames.size}-app foreground match"
                }
    ...
    private fun actionArgsHint(action: WorkflowAction): String {
        ...
        val pairs =
            action.args.entries.joinToString(", ") { (k, v) ->
                val str =
                    if (isSensitiveKey(k)) {
                        "***"
                    } else {
    ...
    private val SENSITIVE_KEY_PARTS =
        setOf(
            "token",
            "password",
            "passphrase",
