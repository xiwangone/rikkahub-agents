...
class BugReportBuilder(
    ...
    private fun captureLogcat(): String {
        ...
        return try {
            val proc =
                ProcessBuilder("logcat", "-d", "-t", "5000", "-v", "threadtime")
...
object SecretRedactor {
    private val patterns: List<Pair<Regex, String>> =
        listOf(
            ...
            Regex(
                """(?i)(authorization|proxy-authorization|x-api-key|x-api-token|x-auth-token|x-access-token|cookie|set-cookie)\s*[:=]\s*(?:Bearer\s+|Token\s+)?[^\r\n,;]+""",
            ) to
                "$1: [redacted]",
